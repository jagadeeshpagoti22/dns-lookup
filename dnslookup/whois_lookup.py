...
class WhoisResult:
    ...
    creation_date: datetime | list | None = None
    expiration_date: datetime | list | None = None
    updated_date: datetime | list | None = None
...
def print_whois_result(result: WhoisResult) -> None:
    """Display WHOIS result in a clean, professional layout."""
    console.print()

    header = Text()
    header.append(":clipboard: ", style="bold")
    header.append("WHOIS", style="bold cyan")
    header.append(" • ", style="dim")
    header.append(result.domain, style="bold white")
    ...
    if result.status:
        status_display = result.status[0] if len(result.status) == 1 else f"{len(result.status)} statuses"
    ...
    if result.name_servers:
        ...
        if len(result.name_servers) > 4:
            table.add_row("", f"[dim]... and {len(result.name_servers) - 4} more[/dim]")
...
def whois_to_json(result: WhoisResult) -> str:
    ...
    data: dict[str, Any] = {
        ...
        "creation_date": serialize_date(result.creation_date),
        "expiration_date": serialize_date(result.expiration_date),
        "updated_date": serialize_date(result.updated_date),
